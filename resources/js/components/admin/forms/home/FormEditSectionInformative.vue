<template>
  <div>
    <v-form :action="urlForm" method="POST" enctype="multipart/form-data">
      ...
      <p class="headline">Edit Section Informative Home: "{{ title }}"</p>
      <v-container>
        <v-row>
          <v-col cols="12">
            <v-text-field
...
export default {
  ...
  created() {
    ...
    this.slogan_item = this.slogan;
    this.description_item = this.description;
    this.footer_item = this.footer;
    this.url_guide_english_pdf = this.btnHrefEnglish;
    this.url_guide_spanish_pdf = this.btnHrefSpanish;
  },
};
</script>
