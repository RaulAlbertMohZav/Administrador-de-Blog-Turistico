<template>
  <v-row justify="center">
    <v-dialog v-model="showDialog" persistent max-width="90%">
      <v-card>
        <v-card-title>
          <span class="text-h5">Create More Info Item</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-form
              action="/admin/viva-andaz/create/fb/experiences/more-info"
              method="POST"
              id="formSubmitCreateMoreInfoItem"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="closeDialogCreateMoreInfo">
            Close
          </v-btn>
          <v-btn color="blue darken-1" text @click="createMoreInfoItem">
            Save
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  methods: {
    closeDialogCreateMoreInfo() {
      this.$emit("closeDialogCreateMoreInfo");
    },
    createMoreInfoItem() {
      document.getElementById("formSubmitCreateMoreInfoItem").submit();
    },
  },
  watch: {
    filePDF(file) {
