<template>
  <div>
    <v-form :action="urlForm" method="POST" enctype="multipart/form-data">
      ...
      <p class="headline">Edit Experience: "{{ title }}"</p>
      <v-container>
        <v-row>
          <v-col cols="12">
            <v-text-field
    ...
    <v-container>
      ...
      <div class="mt-8">
        <meal-plan-individual
          :items-meal-plan-individual="itemsMealPlanIndividual"
          :id-experience="idExperience"
        >
          <template v-slot:csrf-token>
            <slot name="csrf-token"></slot>
...
<script>
import MoreInfo from "./items/MoreInfo.vue";
import MealPlanIndividual from "./items/MealPlanIndividual.vue";
