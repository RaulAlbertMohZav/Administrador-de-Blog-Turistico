<template>
  <v-row justify="center">
    <v-dialog v-model="showDialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
          <span class="text-h5">Create Menu Item</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-form
              action="/admin/viva-andaz/create/fb/restaurants/menus"
              method="POST"
              id="formSubmitCreateMenuItem"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="closeDialogCreateMenu">
            Close
          </v-btn>
          <v-btn color="blue darken-1" text @click="createMenuItem">
            Save
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  methods: {
    closeDialogCreateMenu() {
      this.$emit("closedialogCreateMenu");
    },
    createMenuItem() {
      document.getElementById("formSubmitCreateMenuItem").submit();
    },
  },
  watch: {
    filePDF(file) {
